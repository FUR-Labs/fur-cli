use crate::helpers::cloning::{
    build_id_remap, clone_all_messages, load_conversation_metadata, make_new_conversation_header,
    update_index, write_new_conversation,
};
...
pub fn run_clone(tid: &str, title: Option<String>) {
    ...
    let (new_convo_id, new_title, timestamp) = make_new_conversation_header(&old_title, title);
