...
pub fn run_jot(args: JotArgs) {
    ...
    let emoji = avatars[&avatar_name].as_str().unwrap_or("🐾").to_string();
    ...
    thread_data["messages"]
        .as_array_mut()
        .unwrap()
        .push(Value::String(message_id.clone()));
    fs::write(
        &thread_path,
    ...
    if parent_id != "null" {
        ...
        if let Ok(parent_str) = fs::read_to_string(&parent_path) {
            if let Ok(mut parent_json) = serde_json::from_str::<Value>(&parent_str) {
                ...
                fs::write(
                    parent_path,
                    serde_json::to_string_pretty(&parent_json).unwrap(),
                )
                .unwrap();
            }
        }
    ...
    println!(
        ...
        &thread_id[..8], &message_id[..8], avatar_name
    );
}
